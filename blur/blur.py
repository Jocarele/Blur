    ...
    for row in range (0,rows):
        for col in range (0,cols):
            ...
            if row == 0 and col ==0:
                ...
                img_out[row,col] += img_out[row,col-1]
            elif col == 0: 
                img_out[row,col] += img_out[row-1,col]
            else:
                img_out[row,col] += img_out[row,col-1] + img_out[row-1,col] - img_out[row-1,col-1]
...
def blur_integral(img,dimension): 
    ...
    for row in range (0,rows):   
        ...
        for col in range (0,cols):
            ...
            img_out[row,col] = img_buffer[row_baixo_dir][col_baixo_dir]
            ...
            if row > i_ht :
                ...
                img_out[row,col] -= img_buffer[row-i_ht-1][col_baixo_dir]
                flag1= True
            else :
            ...
            #caso estiver,o inclui na somatória
            if col > i_wt :
                img_out[row,col] -= img_buffer[row_baixo_dir][col-i_wt-1]
                flag2 = True
            else: 
                menos_w += i_wt -col
            ...
            if(flag1 and flag2):
                img_out[row,col] += img_buffer[row-i_ht-1][col-i_wt-1]
            
            if (row == 100 and col == 50):
                print ((hT-menos_h)*(wT-menos_w),hT-menos_h, wT-menos_w)
            img_out [row,col] /= ((hT-menos_h)*(wT-menos_w))
            #img_out [row,col] /= ((hT)*(wT))
...
def main ():
    ...
    #img_blur_m_cv =  img_cv - img_blur 
    img_blur_m_cv =   img_blur - img_cv  

    #Normaliza a imagem, assim verificando se realmente zerou
    img_norm = img_blur_m_cv.copy()  
    img_norm = cv2.normalize(img_blur_m_cv,None,alpha=0,beta=255,norm_type=cv2.NORM_MINMAX)
    ...
    cv2.imwrite ('05 - cv.png', img_cv*255)
    cv2.waitKey ()
